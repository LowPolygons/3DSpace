! Domain Decomposition - Linked Cells
program vectors
  use mpi
  use iso_fortran_env
  ...
  double precision, dimension(:,:), allocatable :: processorRange, originalParticles
  double precision, dimension(:,:), allocatable :: allDataReceived
  ...
  integer, dimension(:,:,:,:), allocatable :: cellHeads
            double precision, dimension(3) :: cellSize
                     integer, dimension(3) :: numCells, currentParticleCell
  ...
  if (dologging .and. rank == 0) print *, "[OUTP] Finished Particle Communications." 
  ...
  deallocate(originalParticles)
  deallocate(allDataReceived)

  times(3) = elapsed_time()
  if (dologging .and. rank == 0) print "(a40,/)", " [OUTP] Link Cell pair counting finished"
...
contains
  ...
  subroutine sort_own_particles(originalParticles, particlePositions, lowerAndUpper, arraySize)
    implicit none
    
    double precision, dimension(:,:), allocatable, intent(out) :: originalParticles
    ...
    allocate(originalParticles(processTracker, 4))
    arraySize = processTracker

    originalParticles(1:arraySize, 1:4) = processDataHolder(1:arraySize, 1:4)
