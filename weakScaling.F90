...
program vectors
  ...
  upperBound = upperBound*numDomains
    ...
    if (success .and. writeToFiles == "Y") then
      ...
      do counter1 = 1, numParticles
        particlePositions(counter1, 1:3) = [&
          & lowerBound(1) + particlePositions(counter1, 1), &
          & lowerBound(2) + particlePositions(counter1, 2), &
          & lowerBound(3) + particlePositions(counter1, 3)  &
        &]
      end do
  ...
  !print *, rank, lowerAndUpper, numDomains
  do counter1 = 1, size(particlePositions)/4 
    ...
    processDataHolder(processTracker, 1:4) = currParticle(1:4) + RAH(rank, numDomains)

    !print *, rank, processDataHolder(processTracker, 1:4)
    ! end if
 ...
 ! if (rank == 0 .and. logTime == "Y") print *, "[Time] Getting Neighbour Processes:", elapsed_time()
  ...
  ! !Start the timer for the main block here
  ! i = timestamp()
...
contains
    ...
    function RAH(currProcess, axisSplits) result(currPos)
