...
program vectors
	...
	integer :: particleProcRatio, arraySize, oppositeCount
	...
	!print *, rank, arraySize, particlePairCount

	! Pick a direction in each axis: this is the only direction where you request particles from the adjcaents/
	! When you compare particle results, you do not append to the sentProcessData: you ONLY check where YOUR particles interact
	! You collect the particle data from these various processes, and this data is what you send to your neighbours
	! This should eliminate any double counting

	!just going to consistantly go with the positive; sending particles to the left, and checking to the right
	...
	do counter1 = 1, 5, 2!2 !1, 3, 5 ; these are the adjacents which you send data to, add one for the one you receive from
		...
		!if (rank == 0) print *,  lowerAndUpper, lowerAndUpper(counter2)+cutoff, lowerAndUpper(counter2+3)-cutoff 
		do counter3 = 1, size(allDataReceived)/3
			
			currParticle = allDataReceived(counter3, 1:3)
			! if (rank == 0) then
			! 	print *, currParticle, lowerAndUpper(counter2)+cutoff
			! end if
			!if (rank == 0) print*, currParticle
		...
		! The particles to be sent have now been determined. Tell the next proc how much its expected to receive, also receive this and allcoate appropriately
		! Do this twice, one for particles used in checking bounds, and then the ones after

		call MPI_SEND(processTracker, 1, MPI_INTEGER, adjacents(counter1), rank*1*counter2, MPI_COMM_WORLD, ierr)
		numRecieved = 0
		...
		!now the second
		call MPI_SEND(oppositeCount, 1, MPI_INTEGER, adjacents(counter1), rank*5*counter2, MPI_COMM_WORLD, ierr)
		numRecievedAfter = 0
		...
		call MPI_SEND(transferThese(1:processTracker, 1:3), processTracker*3, MPI_REAL, &
		& adjacents(counter1), rank*2*counter2, MPI_COMM_WORLD, ierr)
		...
		call MPI_SEND(transferTheseAfter(1:oppositeCount, 1:3), oppositeCount*3, MPI_REAL, &
		& adjacents(counter1), rank*7*counter2, MPI_COMM_WORLD, ierr)
		...
		allocate(allDataReceived(arraySize+numRecieved+numRecievedAfter, 3))
		!allocate(allDataReceived(arraySize+numRecieved, 3))

		allDataReceived(1:arraySize, 1:3) = sentProcessData(1:arraySize, 1:3)

		allDataReceived(arraySize+1: (arraySize+numRecieved), 1:3) = receivedThese(1:numRecieved, 1:3)
		...
		!IS THIS PERHAPS LOSING DATA FROM PREVIOUS ITERATIONS? CONFIRM PLEASE
		! if (rank == 4) then
		! 	if (counter2 == 3) print *, adjacents, " AAAAAAAAAAAA"
		! 	do counter3 = 1, size(allDataReceived)/3
		! 		print *, allDataReceived(counter3, 1:3)
		! 	end do
		! 	print *, ""
		! 	print *, ""
		! 	print *, ""
		! end if
		! if (rank == 5) then
		! 	!print *, arraySize, numRecieved, numRecievedAfter
		! 	do counter3 = 1, size(allDataReceived)/3
		! 	 	print *, counter2, allDataReceived(counter3, 1:3)
		! 	end do
		! end if 
