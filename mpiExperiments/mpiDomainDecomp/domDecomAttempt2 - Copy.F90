...
program vectors
	...
	!mpi stuff
	integer :: ierr, rank, nprocs, numParticles, numRecieved
	...
	real, dimension(:,:), allocatable :: transferThese, receivedThese, allDataReceived  !for use in each process to sort the data into the left/right, down/up, back/forward regions
	...
	do counter1 = 1, 5, 2 !1, 3, 5 ; these are the adjacents which you send data to, add one for the one you receive from
		...
		! The particles to be sent have now been determined. Tell the proc how much its expected to receive, also receive this and allcoate appropriately
		...
		call MPI_RECV(numRecieved, 1, MPI_INTEGER, adjacents(counter1+1), adjacents(counter1+1)*1*counter2, MPI_COMM_WORLD, status1, ierr)
